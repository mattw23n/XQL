...
// Provided by GeeksForGeeks, Edited by CS201 G2T1
...
public class BTree {
    ...
    // Function to reverse the B-tree
    public ReversedBTree reverse() {
        ReversedBTree reversedTree = new ReversedBTree(t);
        if (root != null) {
            reverseNode(root, reversedTree);
        }
        return reversedTree;
    }

    // Helper function to reverse a node
    private void reverseNode(BTreeNode node, ReversedBTree reversedTree) {
        for (int i = 0; i < node.n; i++) {
            // Insert into the reversed B-Tree, swapping key and value
            KeyValuePair<Object, Object> reversedKV = new KeyValuePair<Object, Object>(node.keys[i].getValue(), (Integer) node.keys[i].getKey());
            reversedTree.insert(reversedKV);

            // Recursively reverse child nodes
            if (!node.leaf) {
                reverseNode(node.children[i], reversedTree);
            }
        }
        if (!node.leaf) {
            reverseNode(node.children[node.n], reversedTree);
        }
    }
